...
import {Reflector} from ".//three/ReflectorV2.js"
...
const amb_light = new THREE.AmbientLight(0xffffff,1.8);
scene.add(amb_light);

const plane_geom = new THREE.PlaneGeometry(10,15);

const use_reflector = true;
    if(use_reflector){
        const floor_reflector = new Reflector( plane_geom, {
            clipBias: 0.003,
            textureWidth: 200,
            textureHeight: 100,
            color: 0x777777
        } );
        floor_reflector.position.y = 0.01;
        floor_reflector.rotateX( - Math.PI / 2 );
        floor_reflector.material.transparent=true;
        floor_reflector.material.uniforms.opacity.value=0.05;
        scene.add( floor_reflector );
    }
